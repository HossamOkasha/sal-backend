...
def paginate(items: list, page: int = 1, per_page: int = 20):
    ...
    meta = {"total": len(items), "current_page": page, "per_page": per_page}
...
def create_app(config=ProductionConfig):
    ...
    def register():
        ...
        new_user = User(first_name, last_name, email, username, password, default_role)
    ...
    def patch_profile():
        ...
        try:
            ...
            if (
                "email" in data
                and User.query.filter_by(email=data["email"]).one_or_none()
            ):
                msg = "Email is already in use"
            elif (
                "username" in data
                and User.query.filter_by(username=data["username"]).one_or_none()
            ):
                msg = "Username is already in use"
            else:
    ...
    def get_questions():
        ...
        questions, meta = paginate(all_questions, request.args.get("page", 1, int))
        return jsonify(
            {
                "success": True,
    ...
    def get_question_answers(question_id):
        ...
        answers, meta = paginate(question.answers, request.args.get("page", 1, int), 4)
        return jsonify(
            {
                "success": True,
        ...
        try:
            if vote == 0:
                ...
                content = 'Your question has new %s "%s"' % (
                    "upvote" if vote == 1 else "downvote",
                    question.content,
                )
    ...
    def delete_question(question_id):
        ...
        if user.id != question.user_id:
            if not requires_permission("delete:questions"):
                raise AuthError(
                    "You don't have " "the authority to delete other users questions",
                    403,
                )
        try:
            question.delete()
    ...
    def delete_answer(answer_id):
        ...
        if user.id != answer.user_id:
            if not requires_permission("delete:answers"):
                raise AuthError(
                    "You don't have " "the authority to delete other users answers", 403
                )
        try:
            answer.delete()
        except Exception:
    ...
    def get_user_questions(username):
        ...
        questions, meta = paginate(user.questions, request.args.get("page", 1, int))
    ...
    def report_question():
        ...
        msg.body = 'user "%s" has reported question "%i"' % (username, question_id)
    ...
    def not_found(error):
        return (
            jsonify({"success": False, "message": error.description, "error": 404}),
            404,
        )
    ...
    def bad_request(error):
        return (
            jsonify({"success": False, "message": error.description, "error": 400}),
            400,
        )
    ...
    def un_processable(error):
        return (
            jsonify({"success": False, "message": error.description, "error": 422}),
            422,
        )
    ...
    def method_not_allowed(error):
        return (
            jsonify({"success": False, "message": error.description, "error": 405}),
            405,
        )
    ...
    def too_large(error):
        return (
            jsonify({"success": False, "message": error.description, "error": 413}),
            413,
        )
    ...
    def handle_auth_error(error):
        return (
            jsonify({"success": False, "message": error.message, "error": error.code}),
            error.code,
        )
    ...
    def db_seed():
        ...
        superamdin.permissions.extend([delete_users, delete_answers, delete_questions])
        general.insert()
        superamdin.insert()
